...
export default function FavouritesPage({ url }: TProps) {
	...
	const getStations = async (): Promise<TStation[]> => {
		const result = await fetch(`${url}/stations/byuuid?uuids=${stationIds.join(',')}`);
	...
	if (stations.data?.length === 0) {
		return (
			<div className='flex flex-col h-full w-full justify-center items-center'>
				<Image alt='astronaut holding a star' src={astro} width={500} height={500} />
				<h2 className='font-bold text-2xl px-5'>Your favourite stations will appear here</h2>
			</div>
		);
	}
