...
export default function HistoryStations({ url }: TProps) {
	const getStations = async ({ queryKey }: { queryKey: string[] }) => {
		...
		const result = await fetch(`${url}/stations/byuuid?uuids=${queryKey.join(',')}`);
	...
	if (stations.data?.length === 0) {
		return (
			<div className='flex flex-col h-full w-full justify-center items-center'>
				<Image alt='astronaut holding a star' src={historyImg} width={500} height={500} />
				<h2 className='font-bold text-2xl px-5'>Stations you have listened to will appear hear</h2>
			</div>
		);
	}
